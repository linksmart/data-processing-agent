 ...
 * 
 */
public class BluetoothManager extends DevicesManager implements
        DiscoveryListener
{
    ...
    private void searchServices() throws IOException, InterruptedException
    {
        ...
        for (SerialPortProfileDevice sppd : devices.values())
        {
            ...
            log.debug("Searching services for the device "
                    + d.getBluetoothAddress());
            da.searchServices(null, new UUID[]
            { serialPortUUID }, d, this);
    ...
    private void discover() throws BluetoothStateException,
    ...
    public void deviceDiscovered(RemoteDevice btDevice, DeviceClass cod)
    {
        try
        {
            log.debug("Device discovered: name="
                    + btDevice.getFriendlyName(false) + " address="
                    + btDevice.getBluetoothAddress() + " class="
        ...
        devices.put(btDevice.getBluetoothAddress(),
                new SerialPortProfileDevice(btDevice));
    }
    ...
    public void servicesDiscovered(int transID, ServiceRecord[] servRecord)
    {
        for (ServiceRecord r : servRecord)
        {
            try
            {
                SerialPortProfileDevice device = devices.get(r.getHostDevice()
                        .getBluetoothAddress());
                device.setConnectionUrl(r.getConnectionURL(
                ...
                if (!device.getBtRemoteDevice().isAuthenticated())
                {
                    ...
                    // RemoteDeviceHelper.authenticate(device.getBtRemoteDevice(),
                    // "111111");
                    // while(!device.getBtRemoteDevice().isAuthenticated());
                    // log.info("Device authenticated");
                }

                String className = prop.get(device.getBtRemoteDevice()
                        .getBluetoothAddress());
                // se non ho configurato il tipo di device nelle properties non
                // fa nulla
                ...
                if (!this.devicesDiscovered.containsKey(device
                        .getBtRemoteDevice().getBluetoothAddress()))
                {
                    List<Device> ld = new ArrayList<>();
                    this.devicesDiscovered.put(device.getBtRemoteDevice()
                            .getBluetoothAddress(), ld);
                }
                devicesDiscovered.get(
                        device.getBtRemoteDevice().getBluetoothAddress())
                        .add(d);
                ...
                log.debug("Services discovered: device="
