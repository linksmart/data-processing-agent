 ...
 * 
 */
public class SerialManager extends DevicesManager implements
        SerialPortEventListener
{
    ...
    public void run()
    {
        ...
        for (String port : idsPort.keySet())
        {
            try
            {
                ...
                portObject.openPort();
                portObject.setParams(9600, SerialPort.DATABITS_8,
        ...
        for (String deviceId : configuredDevices.keySet())
        {
            try
            {
                ...
                Constructor<?> constructor = c
                        .getConstructor(SerialManager.class);
                ...
                devicesDiscovered.get(deviceId).add(d);
    ...
    public void dispatchMessage(String cs)
    {
        try
        {
            ...
            //modificato perchè struttura dati di devicesDiscovered è diventata Map<String,List<Device>>
            BaseSerialDevice d = (BaseSerialDevice) devicesDiscovered
                    .get(data[0]).get(0);
            if (d == null)
            {
                return;
