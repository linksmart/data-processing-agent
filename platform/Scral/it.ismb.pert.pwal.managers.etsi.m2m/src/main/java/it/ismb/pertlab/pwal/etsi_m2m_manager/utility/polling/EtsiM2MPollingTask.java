...
public class EtsiM2MPollingTask extends PWALPollingTask<EtsiM2MManager,ContentInstances>
{
    // PWAL XML utility
    ...
    public void run()
    {
        logger.debug("Polling " + this.manager.getActiveSubscriptionsSize()
                + " subscription...");
        if (this.manager.getActiveSubscriptionsSize() > 0)
        {
                    ...
                    .getContentInstancesList().keySet())
            {
                ...
                {
                    // this.httpClient = new PwalHttpClient();
                    logger.debug("Retrieving {} content instances from {}", key,
                    ...
                    logger.debug("Received response after {} millis.",
                    ...
                    {
                        logger.warn("Received a null input stream");
                    }
                    // Thread.sleep(15000);
                }
                                                                             ...
                                                                             // e)
                {
                    logger.error("EtsiM2MPolling task exception: ", e);
                }
            }
        }
    }
