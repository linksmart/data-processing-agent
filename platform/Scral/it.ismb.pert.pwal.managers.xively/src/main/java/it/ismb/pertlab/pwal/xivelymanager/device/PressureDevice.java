 ...
 *
 */
public class PressureDevice implements PressureSensor, DataUpdateSubscriber<Datastream> {
	 ...
	 */
	public PressureDevice(Integer feedID, String streamID) {
	...
	public void handleUpdate(Datastream stream) {
		if(stream.getValue()!=null) {
			...
			if(stream.getValue() != null && !stream.getValue().isEmpty())
			    this.pressure =  Double.valueOf(stream.getValue());
