...
import it.ismb.pertlab.pwal.linksmart.cnet.jaxb.Meta;
...
public class IoTEntityFactory
{
    ...
    public IoTEntity device2IoTEntity(Device device)
    {
        ...
        iotEntity
                .setName(device.getId() + " (" + device.getNetworkType() + ")");
        iotEntity.setAbout(String.format("%s:%s:%s", device.getPwalId(),
        ...
        case DeviceType.WASTE_BIN:
            ...
            setIoTProperty(wb, iotEntity, "Fill level value", "getFillLevel",
                    wb.getFillLevel().getClass().getSimpleName(), "%");
            ...
            String unitSymbol)
    {
        ...
        occupancyProperty.setAbout(String.format("%s:%s", device.getId(),
                device.getType()));
