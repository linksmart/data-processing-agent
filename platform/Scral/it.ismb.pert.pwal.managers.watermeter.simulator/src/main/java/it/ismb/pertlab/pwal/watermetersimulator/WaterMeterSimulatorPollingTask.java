 ...
 */
public class WaterMeterSimulatorPollingTask extends PWALPollingTask<WaterMeterSimulatorManager,WaterMeterSensorData> implements Runnable
{
	/**
	 ...
	 * 
	 * @param manager
	 *            The {@link WaterMeterSimulatorManager} controlling this polling
	 *            task.
	 * @param logger
	 *            The {@link Logger} instance used to log relevant information
	 *            generated by this task.
	 */
	public WaterMeterSimulatorPollingTask(PollingDevicesManager<WaterMeterSensorData> manager, Logger logger)
	{
	...
	public void run()
	{
		...
		if (this.manager.getActiveSubscriptionsSize() > 0)
		{
			...
			Vector<WaterMeterSensorData> updates = ((WaterMeterSimulatorManager)this.manager).getNetworkLayer().getLatestUpdates();
			...
			{
				WaterMeterSensorData cUpdate  = updates.elementAt(i);
				...
				// the given lUID
				Set<DataUpdateSubscription<WaterMeterSensorData>> subscriptionBucket = this.manager
				...
				if (subscriptionBucket != null)
				{
					...
					for (DataUpdateSubscription<WaterMeterSensorData> subscription : subscriptionBucket)
					{
