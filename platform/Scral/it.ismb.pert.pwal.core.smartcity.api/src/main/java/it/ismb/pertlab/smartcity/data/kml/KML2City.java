...
public class KML2City
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		// handle options
		Options options = new Options(new String[] { "-p prefix", "prefix", "-u ontology_url", "ontology_url", "-v",
				"verbose", "-o file", "output file", "-c file", "city KMLfolder or file", "-d file",
				"district KML folder or file", "-q file", "quarter KML folder or file", "-b file",
				"bins KML folder or file", "-t file", "templates folder", "-m mode", "mode, either n3 or jsonld"},
				"kml2city", args);
		...
		if ((options.getValue('c') != null) && (options.getValue('d') != null) && (options.getValue('q') != null)
				&& (options.getValue('m') != null) && (options.getValue('b') != null)
				&& (options.getValue('t') != null) && (!options.getValue('c').isEmpty())
				&& (!options.getValue('d').isEmpty()) && (!options.getValue('q').isEmpty())
				&& (!options.getValue('b').isEmpty()) && (!options.getValue('m').isEmpty())
				&& (!options.getValue('m').isEmpty()) && (!options.getValue('t').isEmpty()))
		{
			...
			KML2City generator = new KML2City(options.getValue('c'), options.getValue('d'), options.getValue('q'),
					options.getValue('b'), binToClass, '.', options.getValue('t'));
			...
			// handle mode
			if (options.getValue('m').equalsIgnoreCase("n3"))
			{
				// render as n3
		...
		{
			options.usage(System.err, "kml2city2");
		}
	}
}
