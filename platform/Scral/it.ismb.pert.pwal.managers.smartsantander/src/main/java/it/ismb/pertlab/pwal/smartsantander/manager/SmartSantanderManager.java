 ...
 *
 */
public class SmartSantanderManager extends
        PollingDevicesManager<SmartSantanderTrafficIntensityJson>
{
    ...
    public void run()
    {
        while (!t.isInterrupted())
        {
            ...
            List<SmartSantanderSingleNodeJson> availableNodes = restClient
                    .getNodes();
            if (availableNodes != null && availableNodes.size() != 0)
            {
                ...
                for (List<Device> ld : devicesDiscovered.values())
                {
                    for (Device d : ld)
                    {
                        ...
                        for (SmartSantanderSingleNodeJson smartSantanderSingleNodeJson : availableNodes)
                        {
                            ...
                            if (d.getId().equals(
                                    smartSantanderSingleNodeJson.getNodeId()))
                            {
                                found = true;
                                break;
                            }
                        ...
                        if (!found)
                        {
                            ...
                            log.info(
                                    "Device {} seems to be removed. It is not into the devices list.",
                                    d);
                            toBeRemoved.add(d);
                ...
                if (toBeRemoved.size() != 0)
                {
                    for (Device device : toBeRemoved)
                    {
                        ...
                        this.devicesDiscovered.get(device.getId()).remove(
                                device);
                        for (DeviceListener dl : this.deviceListener)
                        {
                            dl.notifyDeviceRemoved(device);
                        }
                        // removing subscription
                        for (DataUpdateSubscription<SmartSantanderTrafficIntensityJson> subscription : this.lowLevelDataSubscriptions
                                .get(device.getId()))
                ...
                for (SmartSantanderSingleNodeJson smartSantanderSingleNodeJson : availableNodes)
                {
                    if (!this.devicesDiscovered
                            .containsKey(smartSantanderSingleNodeJson
                                    .getNodeId()))
                    {
                        ...
                        case SmartSantaderDeviceTypes.VEHICLE_COUNTER:
                            ...
                            vehicleCounter.setId(smartSantanderSingleNodeJson
                                    .getNodeId());
                            Location location = new Location();
                            location.setLat(smartSantanderSingleNodeJson
                                    .getLatitude());
                            location.setLon(smartSantanderSingleNodeJson
                                    .getLongitude());
                                            ...
                                            .getNodeId()))
                            {
                                ...
                                this.devicesDiscovered.put(
                                        smartSantanderSingleNodeJson
                                                .getNodeId(), ld);
                                log.debug("Adding subscription for:"
                                        + vehicleCounter.getId());

                                // add device polling subscription
                                // TODO: define sampling time at the device
                                // level
                                this.addSubscription(new DataUpdateSubscription<SmartSantanderTrafficIntensityJson>(
                                        60000, vehicleCounter, vehicleCounter
                                                .getId()));
                            }
                            this.devicesDiscovered.get(
                                    smartSantanderSingleNodeJson.getNodeId())
                                    .add(vehicleCounter);
                            for (DeviceListener l : deviceListener)
                            {
                        ...
                        case SmartSantaderDeviceTypes.VEHICLE_SPEED:
                            SmartSantanderVehicleSpeedDevice vehicleSpeed = new SmartSantanderVehicleSpeedDevice(
                                    this.restClient, this.getNetworkType());
                            vehicleSpeed.setId(smartSantanderSingleNodeJson
                                    .getNodeId());
                            Location location2 = new Location();
                            location2.setLat(smartSantanderSingleNodeJson
                                    .getLatitude());
                            location2.setLon(smartSantanderSingleNodeJson
                                    .getLongitude());
                                            ...
                                            .getNodeId()))
                            {
                                ...
                                // level
                                this.addSubscription(new DataUpdateSubscription<SmartSantanderTrafficIntensityJson>(
                                        60000, vehicleSpeed, vehicleSpeed
                                                .getId()));
                            }
                            this.devicesDiscovered.get(
                                    smartSantanderSingleNodeJson.getNodeId())
                        ...
                        default:
                            log.error(
                                    "I don't know the hell is {}. Unknown type, sorry.",
