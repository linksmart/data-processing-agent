package it.ismb.pertlab.pwal.linksmart.cnet.utilities;
...
public class IoTEntityFactory {
	 ...
	 * 
	 * @param device
	 *            the device object that has to be converted
	...
	public IoTEntity device2IoTEntity(Device device) {
		...
		iotEntity.setName(device.getId());
		iotEntity.setAbout(String.format("%s%s", "_", device.getPwalId())
				.replace('-', '_'));
		...
		case DeviceType.METER:
			...
			setIoTProperty(m, iotEntity, "Istant power", "getPower", m
					.getPower().getClass().getName(), "W");
			setIoTProperty(m, iotEntity, "Is on", "getIsOn", m.isOn().getClass()
					.getName(), "true/false");
			break;
		case DeviceType.WATER_PUMP:
			WaterPump wp = (WaterPump) device;
			setIoTProperty(wp, iotEntity, "Water pump speed", "getSpeed", wp
					.getSpeed().getClass().getName(), "m^3/s");
			break;
		...
		case DeviceType.PHILIPS_HUE:
			PhilipsHue ph = (PhilipsHue) device;
			setIoTProperty(ph, iotEntity, "Lamp on/off status", "getIsOn", ph
					.isOn().getClass().getSimpleName(), "on/off");
			setIoTProperty(ph, iotEntity, "Lamp RGB color status",
					"getRGBColor", String.class.getSimpleName(), "RGB");
			setIoTProperty(ph, iotEntity, "Lamp brightness status",
					"getBrightness", ph.getBrightness().getClass()
							.getSimpleName(), "#");
			setIoTProperty(ph, iotEntity, "Lamp saturation status",
					"getSaturation", ph.getSaturation().getClass()
							.getSimpleName(), "#");
			setIoTProperty(ph, iotEntity, "Lamp hue status", "getHue", ph
					.getHue().getClass().getSimpleName(), "#");
			break;
		default:
			break;
		}
	 ...
	 * 
	 * @param device
	 *            is the reference to the device
	 * @param iotEntity
	 *            is the existing IoTEntity
	 * @param propertyName
	 *            is the choosen name for the property
	 * @param propertyTypeOf
	 *            is the name of the method called to obtain the value to be
	 *            inserted into the IoTProperty
	 * @param dataType
	 *            is the type of the data to be insert into the IoTProperty
	 */
	private void setIoTProperty(Device device, IoTEntity iotEntity,
		...
		// !device.getUnit().getSymbol().isEmpty())
