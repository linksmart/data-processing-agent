		...
		if (existingIoTEntity != null) {
			...
			for (IoTProperty p : existingIoTEntity.getIoTProperty()) {
				...
				case DeviceType.FILL_LEVEL_SENSOR: // Serial manager devices
					...
					break;
				case DeviceType.SIMPLE_FILL_LEVEL_SENSOR: // Serial manager
															// devices
					// must be updated
					switch (p.getName()) {
					case "Level value":
				...
				case DeviceType.PHILIPS_HUE:
					switch (p.getName()) {
					case "Lamp on/off status":
						observation.setValue(String.valueOf(event.getValues()
								.get("getIsOn")));
						break;
					case "Lamp RGB color status":
						observation.setValue(String.valueOf(event.getValues()
								.get("getRGBColor")));
						break;
					case "Lamp brightness status":
						observation.setValue(String.valueOf(event.getValues()
								.get("getBrightness")));
						break;
					case "Lamp saturation status":
						observation.setValue(String.valueOf(event.getValues()
								.get("getSaturation")));
						break;
					case "Lamp hue status":
						observation.setValue(String.valueOf(event.getValues()
								.get("getHue")));
						break;
					default:
