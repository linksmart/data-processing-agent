...
public class ReaderImpl extends UnicastRemoteObject implements LLRPEndpoint, Reader {
    ...
    /** the logger. */
    private static Logger log = Logger.getLogger(ReaderImpl.class.getName());
    ...
    private List<byte[]> inqueue = new LinkedList<byte[]> ();
    
    /** queue to hold the outgoing messages. */
    private List<LLRPMessage> outqueue = new LinkedList<LLRPMessage> ();
    ...
    public void connect(boolean clientInitiatedConnection) throws LLRPRuntimeException, RemoteException {
        ...
        try {
            ...
            if (metaData.getPort() == -1) {
                metaData.setPort(Constants.DEFAULT_LLRP_PORT);
            ...
            if (clientInitiatedConnection) {
                ...
                // run ltk connector.
                LLRPConnector llrpConnector = new LLRPConnector(this, address, metaData.getPort());
                llrpConnector.getHandler().setKeepAliveAck(true);
                llrpConnector.getHandler().setKeepAliveForward(true);
                try {
    ...
    private Runnable getOutQueueWorker() {
        return new OutQueueWorker(outqueue);
    }
    ...
    private Runnable getInQueueWorker() {
        return new InQueueWorker(inqueue);
    }
    
    /**
     * Thread for the WatchDog
     *
     */
    private class WatchDog implements Runnable {
        /**
         * Main thread of the connection watchdog
         */
        public void run() {
    ...
    /**
     * 
     * Thread used to handle an input message
     *
     */
    private class OutQueueWorker implements Runnable {
        LinkedList<LLRPMessage> queue = null;
      

        /**
         * Constructor of the thread that handles output messages
         * 
         * @param queue - <code>List</code> on which send the messages
         */
        public OutQueueWorker(List<LLRPMessage> queue) {
            this.queue = (LinkedList<LLRPMessage>) queue;
        }

        /**
         * Main thread of the thread that manages the output messages
         */
        public void run() {
            try {
                while (true) {
     ...
     *
     */
    private class InQueueWorker implements Runnable {
        LinkedList<byte[]> queue = null;

        /**
         * Constructor of the thread that handles input messages
         * 
         * @param queue - <code>List</code> on which receive the messages
         */
        public InQueueWorker(List<byte[]> queue) {
            this.queue = (LinkedList<byte[]>) queue;
        }

        /**
         * Main thread of the thread that manages the int messages
         */
        public void run() {
            try {
                while (true) {
