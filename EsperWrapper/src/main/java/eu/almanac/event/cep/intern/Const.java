package eu.almanac.event.cep.intern;

/**
 * Created by angel on 12/11/15.
 */
public interface Const extends eu.linksmart.gc.utils.constants.Const {
    public static final String STATEMENT_INOUT_BASE_TOPIC_CONF_PATH ="STATEMENT_INOUT_BASE_TOPIC";
    public static final String DEFAULT_CONFIGURATION_FILE = "conf.cfg";
    public static final String SIMULATION_EXTERNAL_CLOCK = "DataFusionWrapper.Simulation.ExternalClock";

    public static final String SIMULATION_EXTERNAL_CLOCK_STARTING_TIME = "DataFusionWrapper.Simulation.StaringTime";


}
