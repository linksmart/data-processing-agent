...
public class EventMqttObserver extends IncomingMqttObserver {
    ...
    public EventMqttObserver(List<String> topics) {
        ...
        loggerService.info("The Agent(ID:"+ SharedSettings.getId()+") waiting for events from the topic(s): " + topics.stream().map(AgentUtils::topicReplace).collect(Collectors.joining(",")));
